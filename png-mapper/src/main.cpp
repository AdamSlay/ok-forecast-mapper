...
void init_logger(std::string &log_fn) {
    /**
...
void render_data(const std::vector<std::vector<std::string>> &csv ,pngwriter &image, bool isTemp) {
    /**
...
void render_shapes(const std::vector<std::vector<Coordinate>> &shapes, pngwriter &image) {
     ...
     * image: pngwriter image object
     */
    for (std::vector<Coordinate> shape: shapes){
        int x1,y1,x2,y2;
        for (int i = 0; i < shape.size() - 1; i++){
            Coordinate &pair1 = shape.at(i);
            Coordinate &pair2 = shape.at(i+1);
...
void render_text(const std::vector<std::vector<std::string>> &csv, pngwriter &image, bool isTemp) {
    /**
...
int main(int argc, char* argv[]) {
    try {
        ...
        std::vector<std::vector<Coordinate>> shps;
